...
class encoder(nn.Module):
    def __init__(self, output_channels, input_channels, resblocks = True) -> None:
        ...
        self.conv6 = conv_encoder(out_ch*8, output_channels) # 8*8*output_channels
        self.resblock61 = ResnetBlock(output_channels, kernel_size=3)
        
        self.conv7 = conv_encoder(output_channels, output_channels) # 4*4*output_channels
        self.resblock71 = ResnetBlock(output_channels, kernel_size=3)
        
        self.conv8  = nn.Conv2d(output_channels, output_channels, 4, 1, 0) # 1*1*output_channels
    ...
    def forward(self, sharp_image):
        ...
        return encoding.view(-1,self.output_channels), [h1,h2,h3,h4,h5,h6,h7]