...
class Variational_Gen(nn.Module):
    ...
    def update_model(self):
        ...
        loss = 0.3*self.reconstruction_loss_post + self.alignment_loss + \
            self.latent_loss + 0.3 * \
            torch.exp(-1*self.psnr_post) + 0.3*(1-self.ssim_post)
        loss.backward(retain_graph=True)

        self.prior_lstm.zero_grad()
        prior_loss = self.kl_loss_prior + \
            0.3*self.reconstruction_loss_prior + 1.0*self.last_frame_gen_loss + \
            0.3*torch.exp(-1*self.psnr_prior) + 0.3*(1-self.ssim_prior)
